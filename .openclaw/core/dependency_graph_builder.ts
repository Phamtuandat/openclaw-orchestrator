...
export interface DependencyGraph {
  ...
  stats: { total_nodes: number; total_edges: number; cycles: number; orphan_nodes: number };
}
...
export interface ConflictDetectResult {
  ...
  conflictingPairs: Array<{ file1: string; file2: string; reason: string }>;
...
export class DependencyGraphBuilder {
  ...
  private async buildImportEdges(graph: DependencyGraph): Promise<void> {
  ...
  private resolveImport(module: string, fromPath: string): string | null {
    const candidates = [module + '.swift', module.toLowerCase() + '.swift'];
    for (const candidate of candidates) {
      if (this.fileIndex.files[candidate]) return candidate;
    }
    return null;
  }

  private calculateFanoutScores(graph: DependencyGraph): void {
  ...
  private detectCycles(graph: DependencyGraph): Array<{ nodes: string[]; length: number }> {
  ...
  private markCycleRisk(graph: DependencyGraph, cycles: Array<{ nodes: string[]; length: number }>): void {
    const cycleNodeSet = new Set<string>();
    for (const cycle of cycles) {
      for (const node of cycle.nodes) cycleNodeSet.add(node);
    }
    for (const nodePath of cycleNodeSet) {
      graph.nodes[nodePath].cycle_risk = 'high';
    }
    for (const node of Object.values(graph.nodes)) {
      if (node.fanout_score > 5 && node.cycle_risk === 'low') node.cycle_risk = 'medium';
    }
  }

  detectConflicts(tasks: Array<{ agentId: string; targetFiles: string[] }>): ConflictDetectResult {
    const fileToAgents = new Map<string, Set<string>>();
    const conflictingPairs: ConflictDetectResult['conflictingPairs'] = [];
...
export function createDependencyGraph(fileIndex: FileIndex): DependencyGraphBuilder {
